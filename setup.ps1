...
function Install-Git {
    ...
    } else { Write-Host "✅ Git is ready." -ForegroundColor Green }
}
...
function Install-Node {
    if (-not (Test-Path "$nodeDir\node.exe")) {
        ...
        if ($null -ne $extractedFolder -and -not [string]::IsNullOrWhiteSpace($extractedFolder.FullName)) {
            ...
            Write-Host "❌ CRITICAL ERROR: Could not find extracted Node.js folder." -ForegroundColor Red; Exit
        }
...
if ($i) {
    ...
    if ($tool -notin @('git', 'node', 'all')) {
        Write-Host "❌ Invalid tool: '$i'. Please use 'git', 'node', or 'all'." -ForegroundColor Red
        Exit
    }
...
# [Option 4] SMART DEFAULT MODE (No Arguments & Interactive Menu)
    ...
    # ใช้ @(...) เพื่อบังคับให้เป็น Array เสมอ จะได้นับ .Count ได้ถูกต้อง
    ...
    if ($localProjects.Count -eq 0) {
        Write-Host "ℹ️ No project found in 'workspace' folder." -ForegroundColor Yellow
        Show-Help
        Exit
    } 
    elseif ($localProjects.Count -eq 1) {
        ...
        # กรณีมีมากกว่า 1 โปรเจกต์ แสดงเมนูให้เลือก
        ...
        if ($choice -match '^\d+$') {
            ...
            if ($num -eq 0) {
                ...
                Write-Host "`n✅ Selected: $($localProjects[$num - 1].Name)" -ForegroundColor Cyan
            } else {
